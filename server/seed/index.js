...
/**
 * Run the seeder in-process (also used on first boot by server/index.js —
 * must NOT spawn a child `node` process, which may not exist on PATH in the
 * portable build).
 */
function runSeed(options = {}) {
  if (options.reset) resetSchema();
  ...
  // ================================================================ DEMO DATA
  if (options.force || options.reset || db.prepare('SELECT COUNT(*) c FROM cases').get().c === 0) {
    // ---------------- clients
    const clients = {};
    {
      ...
      clients['Méditerranée Distribution'] = qc.run(
...
if (require.main === module) {
  runSeed({
    reset: _argv.includes('--reset'),
    force: _argv.includes('--force') || _argv.includes('--reset'),
    run: _argv.includes('--run')
  });
